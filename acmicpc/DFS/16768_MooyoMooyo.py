...
# Convert string to char, save it in list
...
while True:
    ...
    for i in range(N):
        for j in range(10):
            if M[i][j] is '0' or ck[i][j]:
                continue
            res = dfs(i, j)  # counting
            if res >= K:
                dfs2(i, j, M[i][j])  # remove
                exist = True
    if not exist:
        break
    down()
